...
class Endpoint extends events.EventEmitter {
    ...
    static _onPromisedEvent(promise, cb) {
...
class ChatEndpoint extends endpoint_1.Endpoint {
    ...
    newMessage(msg, socket$$1) {
        return this.privateAPI.users
            .selfInfo(socket$$1)
            .then(user => {
            this._broadcastEvent('new message', typeof msg === 'object' ? Object.assign({}, msg, { name: user.name }) : { name: user.name, text: msg }, socket$$1);
        });
    }
    _onClientConnect(socket$$1) {
        super._onClientConnect(socket$$1);
        socket$$1.on('new message', (msg, cb) => endpoint_1.Endpoint._onPromisedEvent(this.newMessage(msg, socket$$1), cb));
    }
}
...
class UsersEndpoint extends endpoint_1.Endpoint {
    ...
    _onClientConnect(socket$$1) {
        super._onClientConnect(socket$$1);
        socket$$1
            .on('self info', cb => endpoint_1.Endpoint._onPromisedEvent(this.selfInfo(socket$$1), cb))
            .on('add self', (user, cb) => endpoint_1.Endpoint._onPromisedEvent(this.addSelf(user, socket$$1), cb))
            .on('remove self', cb => endpoint_1.Endpoint._onPromisedEvent(this.removeSelf(socket$$1), cb));
    }
}
