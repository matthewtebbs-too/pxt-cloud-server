...
class Endpoint extends events.EventEmitter {
    ...
    _onClientDisconnect(socket$$1) {
        setTimeout(() => __awaiter(this, void 0, void 0, function* () { return yield this._uninitializeClient(socket$$1); }));
    }
}
    ...
    pushDataDiff(name, diff, socket$$1) {
        return __awaiter(this, void 0, void 0, function* () {
            if (diff && diff.length > 0) {
                yield this._pushDataDiff(name, pxtCloudApi.DataRepo.encodeArray(diff));
            }
        });
    }
            ...
            if (success) {
                if (socket$$1) {
                    socket$$1
                        ...
                        .on(pxtCloudApi.Events.WorldPushAllData, (tencdata, cb) => endpoint_.Endpoint._fulfillReceivedEvent(this._pushAllData(tencdata, socket$$1), cb))
                        .on(pxtCloudApi.Events.WorldPushData, ({ name, encdata }, cb) => endpoint_.Endpoint._fulfillReceivedEvent(this._pushData(name, encdata, socket$$1), cb))
                        .on(pxtCloudApi.Events.WorldPushDataDiff, ({ name, encdiff }, cb) => endpoint_.Endpoint._fulfillReceivedEvent(this._pushDataDiff(name, encdiff, socket$$1), cb))
