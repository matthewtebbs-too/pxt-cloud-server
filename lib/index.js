...
class ChatEndpoint extends endpoint_1.Endpoint {
    ...
    newMessage(msg, socket$$1) {
        return this.endpoints.users
            ...
            this._broadcastNotifyEvent(api_1$1.API.Events.ChatNewMessage, Object.assign({}, msg, { name: user.name }), socket$$1);
        });
    }
    _onClientConnect(socket$$1) {
        super._onClientConnect(socket$$1);
        socket$$1.on(api_1$1.API.Events.ChatNewMessage, (msg, cb) => endpoint_1.Endpoint._fulfillReceivedEvent(this.newMessage(msg, socket$$1), cb));
    }
}
...
class UsersEndpoint extends endpoint_1.Endpoint {
    ...
    addSelf(user, socket$$1) {
        return new Promise((resolve, reject) => {
            ...
            multi.exec((error, reply) => {
                ...
                if (!existed) {
                    this._broadcastNotifyEvent(api_1$1.API.Events.UserJoined, userId, user, socket$$1);
                }
                resolve(existed);
            });
        });
    ...
    removeSelf(socket$$1) {
        return new Promise((resolve, reject) => {
            ...
            this.redisClient.del(userkey, (error, reply) => {
                ...
                const existed = !!reply;
                if (existed) {
                    this._broadcastNotifyEvent(api_1$1.API.Events.UserLeft, userId, socket$$1);
                }
                resolve(existed);
            });
        });
    ...
    _onClientConnect(socket$$1) {
        super._onClientConnect(socket$$1);
        socket$$1
            .on(api_1$1.API.Events.UserSelfInfo, cb => endpoint_1.Endpoint._fulfillReceivedEvent(this.selfInfo(socket$$1), cb))
            .on(api_1$1.API.Events.UserAddSelf, (user, cb) => endpoint_1.Endpoint._fulfillReceivedEvent(this.addSelf(user, socket$$1), cb))
            .on(api_1$1.API.Events.UserRemoveSelf, cb => endpoint_1.Endpoint._fulfillReceivedEvent(this.removeSelf(socket$$1), cb));
    }
}
...
class WorldEndpoint extends endpoint_1.Endpoint {
    constructor(endpoints, redisClient, socketServer) {
        ...
        this._debug = debug;
        this._synceddata = new Map();
    }
    addSyncedData(name, source_) {
    ...
    removeSyncedData(name) {
        return this._synceddata.delete(name);
    }
    syncData(name) {
        return new bluebird((resolve, reject) => {
            const data = this._synceddata.get(name);
            if (!data) {
                reject();
                return;
            }
            const latest = cloneDeep(data.source, data.source.cloner);
            const diff = deepDiff.diff(data.latest || {}, latest);
            if (!diff) {
                reject();
                return;
            }
