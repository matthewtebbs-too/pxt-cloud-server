...
class WorldEndpoint extends endpoint_1.Endpoint {
    ...
    syncDataSource(name) {
        ...
        return diff_ ? this.syncDataDiff(name, diff_, false) : bluebird.resolve([]);
    }
    syncDataDiff(name, diff_, apply = true) {
        return bluebird.mapSeries(diff_, d => new bluebird((resolve, reject) => {
            const datakey = WorldDBKeys.data(name);
            this.redisClient.xadd(datakey, '*', 'diff', d.toString(), (error, reply) => {
                if (error) {
                    reject(error);
                    return;
                }
