...
function namespaceEventAPI(nsp) {
    return `pxt-cloud:${nsp}`;
}
exports.namespaceEventAPI = namespaceEventAPI;
exports.namespaceUsersAPI = namespaceEventAPI('users');
exports.namespaceChatAPI = namespaceEventAPI('chat');
exports.namespaceWorldAPI = namespaceEventAPI('world');
});

var api$1 = unwrapExports(api);
var api_1 = api.namespaceEventAPI;
var api_2 = api.namespaceUsersAPI;
var api_3 = api.namespaceChatAPI;
var api_4 = api.namespaceWorldAPI;

var api$2 = /*#__PURE__*/Object.freeze({
	default: api$1,
	__moduleExports: api,
	namespaceEventAPI: api_1,
	namespaceUsersAPI: api_2,
	namespaceChatAPI: api_3,
	namespaceWorldAPI: api_4
});
...
class Endpoint extends events.EventEmitter {
    constructor(privateAPI, redisClient, socketServer, nsp) {
        super();
...
class ChatEndpoint extends endpoint_1.Endpoint {
    constructor(privateAPI, redisClient, socketServer) {
        super(privateAPI, redisClient, socketServer, API.namespaceChatAPI);
    }
    newMessage(msg, socket$$1) {
        return this.privateAPI.users
            .selfInfo(socket$$1)
            .then(user => {
            if (typeof msg !== 'object') {
                msg = { text: msg };
            }
            this._broadcastNotifyEvent('new message', Object.assign({}, msg, { name: user.name }), socket$$1);
        });
    }
    _onClientConnect(socket$$1) {
        super._onClientConnect(socket$$1);
        socket$$1.on('new message', (msg, cb) => endpoint_1.Endpoint._fulfillReceivedEvent(this.newMessage(msg, socket$$1), cb));
    }
}
...
class UsersEndpoint extends endpoint_1.Endpoint {
    ...
    addSelf(user, socket$$1) {
        return new Promise((resolve, reject) => {
            ...
            multi.exec((error, reply) => {
                ...
                if (!existed) {
                    this._broadcastNotifyEvent('user joined', userId, user, socket$$1);
                }
                resolve(existed);
            });
        });
    ...
    removeSelf(socket$$1) {
        return new Promise((resolve, reject) => {
            ...
            this.redisClient.del(userkey, (error, reply) => {
                ...
                const existed = !!reply;
                if (existed) {
                    this._broadcastNotifyEvent('user left', userId, socket$$1);
                }
                resolve(existed);
            });
        });
    ...
    _onClientConnect(socket$$1) {
        super._onClientConnect(socket$$1);
        socket$$1
            .on('self info', cb => endpoint_1.Endpoint._fulfillReceivedEvent(this.selfInfo(socket$$1), cb))
            .on('add self', (user, cb) => endpoint_1.Endpoint._fulfillReceivedEvent(this.addSelf(user, socket$$1), cb))
            .on('remove self', cb => endpoint_1.Endpoint._fulfillReceivedEvent(this.removeSelf(socket$$1), cb));
    }
}
    ...
    start(port_ = server_config_1$1.ServerConfig.port, host_ = server_config_1$1.ServerConfig.host) {
        ...
        return new Promise((resolve, reject) => {
            ...
            this._httpServer.listen(port_, host_, () => {
                ...
                this._createAPI('users', endpoint_users_1$1.UsersEndpoint);
                this._createAPI('chat', endpoint_chat_1$1.ChatEndpoint);
                this._createAPI('world', endpoint_world_1$1.WorldEndpoint);
