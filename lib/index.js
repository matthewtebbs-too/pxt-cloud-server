...
class WorldEndpoint extends endpoint_1.Endpoint {
    ...
    syncDataDiff(name, diff, apply = true) {
        return diff ? bluebird.mapSeries(Array.isArray(diff) ? diff : [diff], diff_ => new bluebird((resolve, reject) => {
            const datakey = WorldDBKeys.data(name);
            this.redisClient.xadd(datakey, '*', 'diff', JSON.stringify(diff_), (error, reply) => {
                if (error) {
                    reject(error);
                    return;
                }
