...
var api_world = createCommonjsModule(function (module, exports) {
Object.defineProperty(exports, "__esModule", { value: true });
exports.WorldEvents = {
    addUser: 'user add',
...
var api_world$1 = unwrapExports(api_world);
var api_world_1 = api_world.WorldEvents;

var api_world$2 = /*#__PURE__*/Object.freeze({
	default: api_world$1,
	__moduleExports: api_world,
	WorldEvents: api_world_1
});
...
class Endpoint extends events.EventEmitter {
    ...
    _broadcastEvent(event, ...args) {
        ...
        if (args.length > 0) {
            ...
            if (undefined === last || last instanceof socket) {
                socket$$1 = last;
                args = args.slice(0, -1);
            }
        }
...
var api_world_1$1 = ( api_world$2 && api_world$1 ) || api_world$2;
...
class WorldEndpoint extends endpoint_base_1$1.Endpoint {
    ...
    addUser(user, cb, socket$$1) {
        const userId = endpoint_base_1$1.Endpoint.connectId(socket$$1);
        const userkey = WorldDBKeys.user(userId);
        const multi = this.redisAPI.multi()
            .exists(userkey)
            .hmset(userkey, user);
        return multi.exec(api_base_1$1.ackHandler(reply => {
            const existed = !!reply[0];
            if (!existed) {
                this._broadcastEvent(api_world_1$1.WorldEvents.userJoined, userId, user, socket$$1);
            }
            return existed;
        }, cb));
    }
    removeUser(cb, socket$$1) {
        ...
        return multi.exec(api_base_1$1.ackHandler(reply => {
            const existed = !!reply[0];
            if (existed) {
                this._broadcastEvent(api_world_1$1.WorldEvents.userLeft, userId, socket$$1);
            }
            return existed;
        }, cb));
    }
    _onClientConnect(socket$$1) {
        ...
        socket$$1.on(api_world_1$1.WorldEvents.addUser, (...args) => this.addUser(args[0], args[1], socket$$1));
        socket$$1.on(api_world_1$1.WorldEvents.removeUser, (...args) => this.removeUser(args[0], socket$$1));
    }
}
...
var require$$2 = ( server$2 && server$1 ) || server$2;
