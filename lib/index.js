...
class WorldEndpoint extends endpoint_1.Endpoint {
    ...
    syncDataSource(name) {
        ...
        return diff ? this.syncDataDiff(name, diff) : bluebird.resolve([]);
    }
    syncDataDiff(name, diff, apply, socket$$1) {
        return bluebird.mapSeries(diff, diff_ => new bluebird((resolve, reject) => {
            const datakey = WorldDBKeys.data(name);
            this.redisClient.xadd(datakey, '*', 'diff', diff_.toString(), (error, reply) => {
                if (error) {
                    reject(error);
                    return;
                }
        ...
        })).tap(ids => this._broadcastNotifyEvent(pxtCloudApi.Events.WorldSyncDataDiff, { name, diff }, socket$$1));
    }
    _onClientConnect(socket$$1) {
        super._onClientConnect(socket$$1);
        socket$$1
            .on(pxtCloudApi.Events.WorldSyncDataDiff, ({ name, diff }, cb) => endpoint_1.Endpoint._fulfillReceivedEvent(this.syncDataDiff(name, diff, true, socket$$1), cb));
    }
}
