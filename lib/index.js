...
class WorldEndpoint extends endpoint_base_1$1.Endpoint {
    ...
    addUser(user, id, cb) {
        const multi = client_redis_1$1.ClientRedis.redisAPI.multi()
            .sadd(exports.keys.users, id)
            .hmset(exports.keys.userId(id), user);
        return multi.exec((err, reply) => cb ? cb({ error: err, reply: reply[0] }) : undefined);
    }
    removeUser(id, cb) {
        const multi = client_redis_1$1.ClientRedis.redisAPI.multi()
            .srem(exports.keys.users, id)
            .del(exports.keys.userId(id));
        return multi.exec((err, reply) => cb ? cb({ error: err, reply: reply[0] }) : undefined);
    }
    _onConnection(socket$$1) {
        super._onConnection(socket$$1);
        socket$$1.on('user_add', this.addUser);
        socket$$1.on('user_remove', this.removeUser);
    }
}
