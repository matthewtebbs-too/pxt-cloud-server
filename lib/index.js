...
Object.defineProperty(exports, "__esModule", { value: true });
const debug = require$$0('pxt-cloud::server:api');
var Events;
(function (Events) {
...
class ChatEndpoint extends endpoint_1.Endpoint {
    ...
    newMessage(msg, socket$$1) {
        return this.endpoints.users
            ...
            this._broadcastNotifyEvent(API.Events.ChatNewMessage, Object.assign({}, msg, { name: user.name }), socket$$1);
        });
    }
    _onClientConnect(socket$$1) {
        super._onClientConnect(socket$$1);
        socket$$1.on(API.Events.ChatNewMessage, (msg, cb) => endpoint_1.Endpoint._fulfillReceivedEvent(this.newMessage(msg, socket$$1), cb));
    }
}
...
class UsersEndpoint extends endpoint_1.Endpoint {
    ...
    addSelf(user, socket$$1) {
        return new Promise((resolve, reject) => {
            ...
            multi.exec((error, reply) => {
                ...
                if (!existed) {
                    this._broadcastNotifyEvent(API.Events.UserJoined, userId, user, socket$$1);
                }
                resolve(existed);
            });
        });
    ...
    removeSelf(socket$$1) {
        return new Promise((resolve, reject) => {
            ...
            this.redisClient.del(userkey, (error, reply) => {
                ...
                const existed = !!reply;
                if (existed) {
                    this._broadcastNotifyEvent(API.Events.UserLeft, userId, socket$$1);
                }
                resolve(existed);
            });
        });
    ...
    _onClientConnect(socket$$1) {
        super._onClientConnect(socket$$1);
        socket$$1
            .on(API.Events.UserSelfInfo, cb => endpoint_1.Endpoint._fulfillReceivedEvent(this.selfInfo(socket$$1), cb))
            .on(API.Events.UserAddSelf, (user, cb) => endpoint_1.Endpoint._fulfillReceivedEvent(this.addSelf(user, socket$$1), cb))
            .on(API.Events.UserRemoveSelf, cb => endpoint_1.Endpoint._fulfillReceivedEvent(this.removeSelf(socket$$1), cb));
    }
}
