...
class WorldEndpoint extends endpoint_1.Endpoint {
    ...
    syncDataSource(name) {
        ...
        return diff ? this.syncDataDiff(name, diff, false) : bluebird.resolve([]);
    }
    syncDataDiff(name, diff, apply = true) {
    ...
    _onClientConnect(socket$$1) {
        super._onClientConnect(socket$$1);
        socket$$1
            .on(pxtCloudApi.Events.WorldSyncDataDiff, ({ name, diff }, cb) => endpoint_1.Endpoint._fulfillReceivedEvent(this.syncDataDiff(name, diff), cb));
    }
}
