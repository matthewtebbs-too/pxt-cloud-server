...
class Endpoint extends events.EventEmitter {
    ...
    static _bufferPromiseHandler(resolve, reject) {
        return Endpoint._promiseHandler(reply => {
            reply ? resolve(Buffer.from(reply, 'binary')) : resolve();
        }, reject);
    }
    static _buffersPromiseHandler(resolve, reject) {
        return Endpoint._promiseHandler(reply => {
...
class WorldEndpoint extends endpoint_1.Endpoint {
    ...
    pullAllData(socket$$1) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this._allPersistedData();
        });
    }
    pullData(name, socket$$1) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this._persistedData(name);
        });
    }
    ...
    pushDataDiff(name, diff, socket$$1) {
        return __awaiter(this, void 0, void 0, function* () {
            yield this._persistDiff(name, diff);
            yield this._notifyEvent(pxtCloudApi.Events.WorldPushDataDiff, { name, diff }, socket$$1);
        });
    }
    _initializeClient(socket$$1) {
        const _super = name => super[name];
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (success) {
                if (socket$$1) {
                    socket$$1
                        .on(pxtCloudApi.Events.WorldPullAllData, cb => endpoint_1.Endpoint._fulfillReceivedEvent(this.pullAllData(socket$$1), cb));
                    socket$$1
                        .on(pxtCloudApi.Events.WorldPullData, (name, cb) => endpoint_1.Endpoint._fulfillReceivedEvent(this.pullData(name, socket$$1), cb));
                    socket$$1
                        .on(pxtCloudApi.Events.WorldPushAllData, cb => endpoint_1.Endpoint._fulfillReceivedEvent(this.pushAllData(socket$$1), cb));
                    socket$$1
                        .on(pxtCloudApi.Events.WorldPushData, (name, cb) => endpoint_1.Endpoint._fulfillReceivedEvent(this.pushData(name, socket$$1), cb));
                    socket$$1
                        .on(pxtCloudApi.Events.WorldPushDataDiff, ({ name, diff }, cb) => endpoint_1.Endpoint._fulfillReceivedEvent(this.pushDataDiff(name, diff, socket$$1), cb));
                }
            }
            return success;
        });
    ...
    _persistedDiff(name) {
        return __awaiter(this, void 0, void 0, function* () {
            return yield new Promise((resolve, reject) => this.redisClient.lrange(WorldDBKeys.dataDiff(name), 0, -1, endpoint_1.Endpoint._buffersPromiseHandler(resolve, reject)));
        });
    }
    _persistDiff(name, diff) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (multi.queue.length >= maxBatchQueue) {
                yield this._persistedData(name);
            }
        });
    }
    _deleteAllPersistedDiff(name) {
        return __awaiter(this, void 0, void 0, function* () {
    ...
    _allPersistedData() {
        return __awaiter(this, void 0, void 0, function* () {
            const datakeys = yield new Promise((resolve, reject) => this.redisClient.keys(WorldDBKeys.data('*'), endpoint_1.Endpoint._promiseHandler(resolve, reject)));
            const result = [];
            for (const datakey of datakeys) {
                const name = WorldDBKeys.nameFromKey(datakey);
                result.push({ name, data: yield this._persistedData(name) });
            }
            return result;
        });
    }
    _persistedData(name) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const current = yield new Promise((resolve, reject) => this.redisClient.get(WorldDBKeys.data(name), endpoint_1.Endpoint._bufferPromiseHandler(reply => resolve(reply ? pxtCloudApi.DataRepo.decode(reply) : {}), reject)));
            const diff = yield this._persistedDiff(name);
            if (diff.length > 0) {
                yield this._persistData(name, pxtCloudApi.DataRepo.applyDataDiff(current, diff));
                yield this._deleteAllPersistedDiff(name);
            }
            return current;
        });
    }
    _persistData(name, data) {
        return __awaiter(this, void 0, void 0, function* () {
            yield new Promise((resolve, reject) => this.redisClient.set(WorldDBKeys.data(name), pxtCloudApi.DataRepo.encode(data).toString('binary'), endpoint_1.Endpoint._promiseHandler(resolve, reject)));
        });
    }
}
