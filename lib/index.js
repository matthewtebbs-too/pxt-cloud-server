...
class ChatEndpoint extends endpoint_1.Endpoint {
    ...
    newMessage(msg, socket$$1) {
        return this.publicAPI.users.selfInfo()
            .then(user => {
            this._broadcastEvent('new message', typeof msg === 'object' ? msg : { name: user.name, text: msg }, socket$$1);
        });
    }
    _onClientConnect(socket$$1) {
        super._onClientConnect(socket$$1);
        socket$$1.on('new message', (msg, cb) => this.newMessage(msg, socket$$1));
    }
}
...
class UsersEndpoint extends endpoint_1.Endpoint {
    ...
    _onClientConnect(socket$$1) {
        super._onClientConnect(socket$$1);
        socket$$1.on('self info', (cb) => this.selfInfo(socket$$1));
        socket$$1.on('add self', (user, cb) => this.addSelf(user, socket$$1));
        socket$$1.on('remove self', (cb) => this.removeSelf(socket$$1));
    }
}
