...
class UsersEndpoint extends endpoint_base_1$1.Endpoint {
    ...
    selfInfo(cb, socket$$1) {
        ...
        return this.redisAPI.hgetall(userkey, api_base_1$1.ackHandler(cb, reply => {
            return {
    ...
    addSelf(user, cb, socket$$1) {
        ...
        return multi.exec(api_base_1$1.ackHandler(cb, reply => {
    ...
    removeSelf(cb, socket$$1) {
        const userId = endpoint_base_1$1.Endpoint.userId(socket$$1);
        const userkey = UsersDBKeys.user(userId);
        return this.redisAPI.del(userkey, api_base_1$1.ackHandler(cb, reply => {
            const existed = !!reply && 1 === reply[0];
            if (existed) {
