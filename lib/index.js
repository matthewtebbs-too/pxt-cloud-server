...
exports.ackHandlerVoid = (cb) => ackHandler(cb)(null, undefined);
...
var api_base_2 = api_base.ackHandlerVoid;
var api_base_3 = api_base.mappedAckHandler;

var api_base$2 = /*#__PURE__*/Object.freeze({
	default: api_base$1,
	__moduleExports: api_base,
	ackHandler: api_base_1,
	ackHandlerVoid: api_base_2,
	mappedAckHandler: api_base_3
});
...
class ChatEndpoint extends endpoint_base_1$1.Endpoint {
    constructor(socketServerAPI, redisAPI) {
        super(socketServerAPI, redisAPI, 'pxt-cloud.chat');
    }
    newMessage(msg, cb, socket$$1) {
        const result = this._broadcastEvent('new message', typeof msg === 'object' ? msg : { text: msg }, socket$$1);
        if (result) {
            api_base_1$1.ackHandlerVoid(cb);
        }
        return result;
    }
    _onClientConnect(socket$$1) {
...
class UsersEndpoint extends endpoint_base_1$1.Endpoint {
    constructor(socketServerAPI, redisAPI) {
        super(socketServerAPI, redisAPI, 'pxt-cloud.users');
    }
    selfInfo(cb, socket$$1) {
        const userId = endpoint_base_1$1.Endpoint.userId(socket$$1);
        const userkey = UsersDBKeys.user(userId);
        return this.redisAPI.hgetall(userkey, api_base_1$1.mappedAckHandler(reply => {
            return {
    ...
    addSelf(user, cb, socket$$1) {
        ...
        return multi.exec(api_base_1$1.mappedAckHandler(reply => {
    ...
    removeSelf(cb, socket$$1) {
        const userId = endpoint_base_1$1.Endpoint.userId(socket$$1);
        const userkey = UsersDBKeys.user(userId);
        return this.redisAPI.del(userkey, api_base_1$1.mappedAckHandler(reply => {
            const existed = !!reply;
            if (existed) {
...
class Server {
    ...
    get chatAPI() {
        return this._endpoints.chat;
    }
    get usersAPI() {
        return this._endpoints.users;
    }
    get worldAPI() {
        return this._endpoints.world;
    }
        ...
        return new Promise((resolve, reject) => {
            ...
            this._httpServer.listen(port_, host_, () => {
                ...
                this._redisClient.connect()
                    .then(client => {
                    this._endpoints = {
                        chat: new endpoint_chat_1$1.ChatEndpoint(this._socketServer.socketAPI, client.redisAPI),
                        users: new endpoint_users_1$1.UsersEndpoint(this._socketServer.socketAPI, client.redisAPI),
                        world: new endpoint_world_1$1.WorldEndpoint(this._socketServer.socketAPI, client.redisAPI),
                    };
                    resolve(this);
                })
