...
class ChatEndpoint extends endpoint_base_1$1.Endpoint {
    ...
    newMessage(msg, cb, socket$$1) {
        const result = this._broadcastEvent('new message', msg, socket$$1);
        if (result) {
...
class UsersEndpoint extends endpoint_base_1$1.Endpoint {
    ...
    _onClientConnect(socket$$1) {
        super._onClientConnect(socket$$1);
        socket$$1.on('add user', (user, cb) => this.addUser(user, cb, socket$$1));
        socket$$1.on('remove user', (cb) => this.removeUser(cb, socket$$1));
    }
}
